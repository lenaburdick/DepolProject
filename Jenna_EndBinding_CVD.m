...
%This code was copy and pasted from Jenna_EndBinding_LVD.m
...
avgDepolRateMatrix(w)=avgDepolRate;
...
% %---Plotting Theory-----
% 
% x=((StartingLength*MonomerMeasurement-y)./(kUnassisted*MonomerMeasurement))*60;
% y=(StartingLength-(0:ShrinkAmount))*MonomerMeasurement;  %times the length of 1 monomer
% Theoryplot=plot(x,y,'*');
% Theoryplot.LineWidth=6;
% Theoryplot.Color=[1.0 0.702 0.118];
...
set(gca, 'YLim',[min(y)*(2/3),max(y)*(3/2)]); %If the graph is too zoomed in, you can't tell its a straight line
