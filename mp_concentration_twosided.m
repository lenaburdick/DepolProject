...
concentrationtimeplot.DisplayName=('Concentration of Protein Versus Time For Protein To Reach Either End')

hold on

xlabel('Concentration (Number of Proteins)')
ylabel('Time Until Protein Reaches Either End (unit time=number of steps)')
title('Time Until Protein Diffuses to Either End of Filament With Varying Protein Concentrations')
set(gca, 'fontsize',20)

hold on

% Time=LengthofFilament^2/12D+1/(LengthofFilament*kon*Concentration)+1/koff
%Concentration=1/(LengthofFilament*kOn)(Time-LengthofFilament^2/12D-1/kOff)

D=1; % CHANGE ONCE YOU CALCULATE DIFFCON

yt=1./((LengthofFilament*kOn)*(NewTotalTimeMatrix-LengthofFilament^2/12D-1/kOff))

theoryplot=plot(x,yt)
