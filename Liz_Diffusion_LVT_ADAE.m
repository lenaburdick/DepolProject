...
%This code was copy and pasted from Jenna_Diffusion_LVT_ADAE.m

%Once a protein is at the end, the filament must depolymerize. This is
%different than some other verisons of the simulation, where proteins can
%walk off of the end.

%The filament has a set number of proteins pre-bound at the beginning of
%the simulation. This is to compensate for the number of steps the
%simulation has to take before many proteins are bound.

%-File Name--------------------------------------------------------------
mainfilename='Liz_Diffusion_LVT_ADAE'; %main file name (parameters added later)
savefigures=0; % 1 if yes, 0 if no
savedata=0; % 1 if yes, 0 if no
...
%-Other Parameters-------------------------------------------------------
NumberofRuns=25;
MinFilamentSize=200; %How small the filament will shrink to
...
%---Measurements-------------------------------
MonomerMeasurement=.032; %This is the length of monomer in um
...
%%% TEST %%%%%%%%%%%%%%%%%%%%%%    
if isequal(sum(ProteinTrackingMatrix(:,6)>0),sum(OccupancyArray(1:NumberofMonomers)),BoundProteins(end))==0
    pause=1; % Pauses the simulation if these values are ever not equal (messing up 06/24/21)
% else
%     isequal(sum(ProteinTrackingMatrix(:,6)>0),sum(OccupancyArray(1:NumberofMonomers)),BoundProteins(end))
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
