    ...
    for i=1:NumberofRuns
        ...
        while IsMotorOnEnd<1
            ...
            if Position==NumberofMonomers %if it's on the final monomer, the loop stops
                ...
                if Position==1 %if it's on the first monomer, the loop stops
                    IsMotorOnEnd=1;
                else
...
EveryRunMatrix(1,:)=0;  % Makes the proteins that start on the end reach the end immedietelty so you don't get weird results with Inf 
...
fileID3 = fopen('p.filamentlength.twosided.cluster.txt','a'); 
