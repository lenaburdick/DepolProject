...
ABValues=coeffvalues(f); %gives a and b
    DiffCons=[(-1)/(2*ABValues(1)) LengthofFilament^2/(2*ABValues(2))]; %Uses LengthofFilament instead of number of monomers because in equation length 1 monomer=length 0
            %Gives the two possible values for the diffusion constant, we use the average as the real constant
 ...
 %begin on (i.e. Average tiime matrix begins on DataToTrack(11) and ends on
...
fileID3 = fopen('sp.startingposition.onesided.cluster.txt','a'); 
